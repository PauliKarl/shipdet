...
def rfastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    """
    Computes the loss for Faster R-CNN.
...
class rRoIHeads(torch.nn.Module):
    def __init__(self,
                 box_roi_pool,
                 box_head,
                 box_predictor,
        ...
        if bbox_reg_weights is None:
            bbox_reg_weights = (10., 10., 5., 5., 5.)
        self.box_coder = det_utils.rBoxCoder(bbox_reg_weights)
    ...
    def check_targets(self, targets):
        assert targets is not None
        assert all("thetaobbes" in t for t in targets)
        assert all("labels" in t for t in targets)
        if self.has_mask:
            assert all("masks" in t for t in targets)
    ...
    def select_training_samples(self, proposals, targets):
        self.check_targets(targets)
        gt_boxes = [t["thetaobbes"] for t in targets]
        gt_labels = [t["labels"] for t in targets]
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        box_features = self.box_roi_pool(features, proposals, image_shapes)
        #print("roi:", box_features)
        box_features = self.box_head(box_features)
        class_logits, box_regression = self.box_predictor(box_features)

        result, losses = [], {}
        if self.training:
            loss_classifier, loss_box_reg = rfastrcnn_loss(
                class_logits, box_regression, labels, regression_targets)
            losses = dict(loss_classifier=loss_classifier, loss_box_reg=loss_box_reg)
        else:
            boxes, scores, labels = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
